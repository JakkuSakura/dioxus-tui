...
fn collect_text(doc: &BaseDocument, id: usize) -> Option<String> {
...
fn render_tree(
    ...
    let area = parent_rect.unwrap_or_else(|| surface.area());
    ...
    let text_opt = collect_text(doc, node_id);

    let mut draw_text = |area: Rect, text: String| {
    ...
    match tag.as_str() {
        ...
        "button" => {
            let label = text_opt.unwrap_or_default();
            let boxed = format!("[{label}]");
            draw_text(rect, boxed);
        }
        "input" => {
            let val = node
                .element_data()
        ...
        _ => {
            ...
            for child in node.children.iter() {
                render_tree(surface, doc, *child, Some(area));
            }
        }
    }
}
