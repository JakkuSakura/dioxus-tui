...
use crate::runtime::{config::Config, style::{convert, RinkColor, RinkStyle}};
...
impl Default for RinkCell {
    fn default() -> Self {
        Self {
            symbol: "".to_string(),
            fg: RinkColor { color: Color::Rgb(0, 0, 0), alpha: 0 },
            bg: RinkColor { color: Color::Rgb(0, 0, 0), alpha: 0 },
...
impl RinkCell {
    pub fn set_style(&mut self, style: RinkStyle) {
        if let Some(c) = style.fg { self.fg = c; }
        if let Some(c) = style.bg { self.bg = c; }
