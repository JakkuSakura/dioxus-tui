...
use crate::runtime::config::RenderingMode;
...
fn parse_value(v: &str, current_max_output: f32, required_max_output: f32) -> Result<f32, ParseFloatError> {
...
fn parse_hex(color: &str) -> Result<Color, ParseColorError> {
    ...
    if color_ok { Ok(Color::Rgb(values[0], values[1], values[2])) } else { Err(ParseColorError) }
}
...
impl FromStr for RinkColor {
    ...
    fn from_str(color: &str) -> Result<Self, Self::Err> {
        match color {
            "red" => Ok(RinkColor { color: Color::Red, alpha: 255 }),
            "black" => Ok(RinkColor { color: Color::Black, alpha: 255 }),
            "green" => Ok(RinkColor { color: Color::Green, alpha: 255 }),
            "yellow" => Ok(RinkColor { color: Color::Yellow, alpha: 255 }),
            "blue" => Ok(RinkColor { color: Color::Blue, alpha: 255 }),
            "magenta" => Ok(RinkColor { color: Color::Magenta, alpha: 255 }),
            "cyan" => Ok(RinkColor { color: Color::Cyan, alpha: 255 }),
            "gray" => Ok(RinkColor { color: Color::Gray, alpha: 255 }),
            "darkgray" => Ok(RinkColor { color: Color::DarkGray, alpha: 255 }),
            "orangered" => Ok(RinkColor { color: Color::LightRed, alpha: 255 }),
            "lightgreen" => Ok(RinkColor { color: Color::LightGreen, alpha: 255 }),
            "lightyellow" => Ok(RinkColor { color: Color::LightYellow, alpha: 255 }),
            "lightblue" => Ok(RinkColor { color: Color::LightBlue, alpha: 255 }),
            "orchid" => Ok(RinkColor { color: Color::LightMagenta, alpha: 255 }),
            "lightcyan" => Ok(RinkColor { color: Color::LightCyan, alpha: 255 }),
            "white" => Ok(RinkColor { color: Color::White, alpha: 255 }),
            _ => {
                if color.len() == 7 && color.starts_with('#') {
                    parse_hex(color).map(|c| RinkColor { color: c, alpha: 255 })
                } else if let Some(stripped) = color.strip_prefix("rgb(") {
                    let color_values = stripped.trim_end_matches(')');
                    if color.matches(',').count() == 3 {
                        let (alpha, rgb_values) = color_values.rsplit_once(',').ok_or(ParseColorError)?;
                        ...
                        parse_rgb(color_values).map(|c| RinkColor { color: c, alpha: 255 })
                    }
                } else if let Some(stripped) = color.strip_prefix("rgba(") {
                    let color_values = stripped.trim_end_matches(')');
                    if color.matches(',').count() == 3 {
                        let (rgb_values, alpha) = color_values.rsplit_once(',').ok_or(ParseColorError)?;
                    ...
                    if color.matches(',').count() == 3 {
                        ...
                        parse_hsl(color_values).map(|c| RinkColor { color: c, alpha: 255 })
                    }
...
pub fn convert(mode: RenderingMode, c: Color) -> Color {
    if let Color::Reset = c {
        ...
        match mode {
            RenderingMode::BaseColors => match c {
...
impl Default for RinkStyle {
    fn default() -> Self {
        Self {
            fg: Some(RinkColor { color: Color::White, alpha: 255 }),
            bg: None,
...
impl RinkStyle {
    ...
    pub fn merge(mut self, other: RinkStyle) -> Self {
        ...
        self.add_modifier(other.add_modifier).remove_modifier(other.sub_modifier)
    }
}
