...
impl CustomElement for Input {
    ...
    fn create(mut root: dioxus_native_core::real_dom::NodeMut) -> Self {
        ...
        let input_type = el
            .attributes
            .get(&OwnedAttributeDiscription { name: "type".to_string(), namespace: None })
            .and_then(|value| value.as_text());
        match input_type
            .map(|type_| type_.trim().to_lowercase())
            .as_deref()
        {
            Some("button") => { drop(node_type); Input::Button(Button::create(root)) }
            Some("checkbox") => { drop(node_type); Input::CheckBox(CheckBox::create(root)) }
            Some("textbox") => { drop(node_type); Input::TextBox(TextBox::create(root)) }
            Some("password") => { drop(node_type); Input::Password(Password::create(root)) }
            Some("number") => { drop(node_type); Input::Number(Number::create(root)) }
            Some("range") => { drop(node_type); Input::Slider(Slider::create(root)) }
            _ => { drop(node_type); Input::TextBox(TextBox::create(root)) }
        }
    }
    ...
    fn attributes_changed(
        ...
        match self {
            ...
            Input::CheckBox(checkbox) => checkbox.attributes_changed(root, attributes),
            Input::TextBox(textbox) => textbox.attributes_changed(root, attributes),
            Input::Password(password) => password.attributes_changed(root, attributes),
            Input::Number(number) => number.attributes_changed(root, attributes),
            Input::Slider(slider) => slider.attributes_changed(root, attributes),
        }
    }
}
...
impl RinkWidget for Input {
    fn handle_event(&mut self, event: &crate::runtime::hooks::Event, node: dioxus_native_core::real_dom::NodeMut) {
        match self {
            ...
            Input::TextBox(textbox) => textbox.handle_event(event, node),
            Input::Password(password) => password.handle_event(event, node),
            Input::Number(number) => number.handle_event(event, node),
            Input::Slider(slider) => slider.handle_event(event, node),
        }
    }
}
