...
use crate::runtime::hooks::{Event, EventData};
use crate::runtime::query::{get_layout, Query};
...
impl Slider {
    fn size(&self) -> f64 { self.max - self.min }
    fn step(&self) -> f64 { self.step.unwrap_or(self.size() / 10.0) }

    fn width(el: &ElementNodeMut) -> String {
        if let Some(value) = el
            .get_attribute(&OwnedAttributeDiscription { name: "width".to_string(), namespace: Some("style".to_string()) })
    ...
    fn height(el: &ElementNodeMut) -> String {
        if let Some(value) = el
            .get_attribute(&OwnedAttributeDiscription { name: "height".to_string(), namespace: Some("style".to_string()) })
            .and_then(|value| value.as_text())
            .map(|value| value.to_string())
        { value } else { "1px".to_string() }
    }
    ...
    fn update_min_attr(&mut self, el: &ElementNodeMut) {
        if let Some(value) = el
            .get_attribute(&OwnedAttributeDiscription { name: "min".to_string(), namespace: None })
            .and_then(|value| value.as_text())
            .map(|value| value.to_string())
    ...
    fn update_max_attr(&mut self, el: &ElementNodeMut) {
        if let Some(value) = el
            .get_attribute(&OwnedAttributeDiscription { name: "max".to_string(), namespace: None })
            .and_then(|value| value.as_text())
            .map(|value| value.to_string())
    ...
    fn update_step_attr(&mut self, el: &ElementNodeMut) {
        if let Some(value) = el
            .get_attribute(&OwnedAttributeDiscription { name: "step".to_string(), namespace: None })
            .and_then(|value| value.as_text())
            .map(|value| value.to_string())
    ...
    fn update_value(&mut self, new: f64) { self.value = new.clamp(self.min, self.max); }
    ...
    fn write_value(&self, rdom: &mut RealDom, id: NodeId) {
        ...
        let world = rdom.raw_world_mut();
        {
            ...
            ctx.send(Event { id, name: "input", data: EventData::Form(data), bubbles: true });
        }
    }
    ...
    fn handle_keydown(&mut self, mut root: NodeMut, data: &SerializedKeyboardData) {
        ...
        let step = self.step();
        match key {
            Key::ArrowDown | Key::ArrowLeft => self.update_value(self.value - step),
            Key::ArrowUp | Key::ArrowRight => self.update_value(self.value + step),
            _ => return,
        }
...
impl CustomElement for Slider {
    ...
    fn create(mut root: dioxus_native_core::real_dom::NodeMut) -> Self {
        ...
        let value = el.attributes.get(&OwnedAttributeDiscription { name: "value".to_string(), namespace: None });
        let value = value
