...
fn footer_line(line: &mut LineBuilder, border: &Style) {
    line.set_str_styled(
        0,
        "╰┘↑ select ↓└┘info ↵└┘terminate└┘kill└┘signals└──────────┘9/4384└╯",
        border.clone(),
    );
}
...
pub fn render(data: &ProcData) -> ComponentBlock {
    ...
    for (idx, row) in data.rows_bottom.iter().enumerate() {
